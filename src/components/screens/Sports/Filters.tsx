    ...
    <div className={clsx(styles.filters, isFilterOpen && styles.filterOpen)}>
        ...
        <div className={styles.tags}>
          ...
          {sports.map(sport => (
            <div key={sport.name}
              ...
              <img src="/sports/ball-blue.svg" alt=""/>
              {sport.name}
              <span>{sport.games?.length || 0}</span>
            </div>
          ))}
        </div>
      ...
      <div className={clsx(styles.classicFilter, filterType === 'classic' && styles.chosenFilter)}>
        ...
        <div onClick={() => router.push('/sports/')}
             className={clsx(styles.filter, pathname === '/sports' && styles.filterActive)}>Top events <span>-</span>
        </div>
        {sports.map(sport => (
            ...
            <div onClick={() => router.push(`/sports/${sport.slug.toLowerCase()}`)}
              ...
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  ...
                  fill={!!pathname.split('/').find(str => str === sport.slug) ? "#ffffff" : "#616161"}/>
              </svg>
              {sport.name} <span>{sport.games?.length || 0}</span>
            </div>
            {!!pathname.split('/').find(str => str === sport.slug) && <div className={styles.leagues}>
              <div onClick={() => router.push(`/sports/${sport.slug.toLowerCase()}`)}
                   className={clsx(styles.filter, pathname === `/sports/${sport.slug}` && styles.filterActive)}>All <span>-</span>
              </div>
              <span/>
              {sport.countries.map(({leagues}) => leagues.map(league => (
                <div key={league.slug} onClick={() => router.push(`/sports/${sport.slug.toLowerCase()}/${league.slug}`)}
