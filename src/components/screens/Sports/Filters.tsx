  ...
  return (
    <div className={clsx(styles.filters, isFilterOpen && styles.filterOpen)}>
      ...
      <div className={clsx(styles.classicFilter, filterType === 'classic' && styles.chosenFilter)}>
        ...
        {sports.map(sport => (
          <div onClick={() => {
            // router.push(`/sports?sport=${sport.name.toLowerCase()}`)
            router.push(`/sports/${sport.slug.toLowerCase()}`)
          }} className={clsx(styles.filter, sport.slug === pathname.split('/sports/').pop() && styles.filterActive)}
               key={sport.name}>
